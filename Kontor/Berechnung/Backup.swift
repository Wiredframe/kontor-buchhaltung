...
enum Backup {
    ...
    static func snapshot(_ context: ModelContext) throws -> Snapshot {
        Snapshot(
            ...
            ausgaben: try context.fetch(FetchDescriptor<ExpenseEntry>()).map {
                AusgabeDTO(
                    datum: $0.datum, bezeichnung: $0.bezeichnung, anbieter: $0.anbieter,
            ...
            einnahmen: try context.fetch(FetchDescriptor<Income>()).map {
                EinnahmeDTO(
                    kunde: $0.kunde, rnNetto: $0.rnNetto, ust: $0.ust,
            ...
            aufgaben: try context.fetch(FetchDescriptor<MonthlyTask>()).map {
                AufgabeDTO(
                    titel: $0.titel, monat: $0.monat, erledigt: $0.erledigt,
            ...
            steuern: try context.fetch(FetchDescriptor<TaxPayment>()).map {
                SteuerDTO(
                    kind: $0.kind, jahr: $0.jahr, faellig: $0.faellig, betrag: $0.betrag,
            ...
            zuordnungsRegeln: try context.fetch(FetchDescriptor<ZuordnungsRegel>()).map {
                ZuordnungsRegelDTO(
                    schluessel: $0.schluessel, kategorie: $0.kategorie,
            ...
            vorlagen: try context.fetch(FetchDescriptor<Vorlage>()).map {
                VorlageDTO(
                    bezeichnung: $0.bezeichnung, anbieter: $0.anbieter, betragBrutto: $0.betragBrutto,
            ...
            importBuchungen: try context.fetch(FetchDescriptor<ImportBuchung>()).map {
                ImportBuchungDTO(
                    schluessel: $0.schluessel, buchungstag: $0.buchungstag, betrag: $0.betrag,
    ...
    static func autoSichern(_ context: ModelContext, behalteTage: Int = 14) {
        ...
        let leererStore =
            ((try? context.fetchCount(FetchDescriptor<YearSettings>())) ?? 0) == 0
        ...
        let alle =
            (try? FileManager.default.contentsOfDirectory(at: ordner, includingPropertiesForKeys: nil))?
    ...
    static func importData(_ data: Data, in context: ModelContext) throws -> (neu: Int, uebersprungen: Int) {
        ...
        for d in snap.jahre {
            ...
            context.insert(
                YearSettings(
                    ...
                    snapshotProMonat: d.snapshotProMonat ?? [:]))
            neu += 1
        }

        var ausgabeBestand = zaehleKeys(
            try context.fetch(FetchDescriptor<ExpenseEntry>()).map { posKey($0.datum, $0.bezeichnung, $0.brutto) })
        for d in snap.ausgaben {
            let k = posKey(d.datum, d.bezeichnung, d.brutto)
            if verbrauche(k, &ausgabeBestand) {
                skip += 1
                continue
            }
            context.insert(
                ExpenseEntry(
                    datum: d.datum, bezeichnung: d.bezeichnung, anbieter: d.anbieter,
        ...
        var vorlageKeys = Set(
            try context.fetch(FetchDescriptor<Vorlage>()).map { $0.bezeichnung.lowercased() + "|" + $0.art.rawValue })
        for d in snap.vorlagen ?? [] {
            let k = d.bezeichnung.lowercased() + "|" + d.art.rawValue
            if vorlageKeys.contains(k) {
                skip += 1
                continue
            }
            ...
            context.insert(
                Vorlage(
                    bezeichnung: d.bezeichnung, anbieter: d.anbieter, betragBrutto: d.betragBrutto,
                    steuerart: d.steuerart, betrieblich: d.betrieblich, art: d.art, umlagefaehig: d.umlagefaehig))
            neu += 1
        }
        ...
        var einBestand = zaehleKeys(
            bestehendeEinnahmen.filter { $0.rechnungsnummer == nil }
                .map { posKey($0.rechnungsdatum, $0.kunde, $0.rnNetto) })
        for d in snap.einnahmen {
            if let nr = d.rechnungsnummer {
                if rnNummern.contains(nr) {
                    skip += 1
                    continue
                }
                rnNummern.insert(nr)
        ...
        var taskKeys = Set(
            try context.fetch(FetchDescriptor<MonthlyTask>()).map {
                $0.titel.lowercased() + "|" + String(Int($0.monat.timeIntervalSince1970))
            })
        for d in snap.aufgaben {
            let k = d.titel.lowercased() + "|" + String(Int(d.monat.timeIntervalSince1970))
            if taskKeys.contains(k) {
                skip += 1
                continue
            }
            taskKeys.insert(k)
            context.insert(
                MonthlyTask(
                    titel: d.titel, monat: d.monat, erledigt: d.erledigt,
                    intervall: d.intervall ?? .einmalig, faelligTag: d.faelligTag ?? 1,
                    quartalsMonate: d.quartalsMonate ?? []))
            neu += 1
        }

        var lmBestand = zaehleKeys(
            try context.fetch(FetchDescriptor<GroceryEntry>()).map { posKey($0.datum, $0.ort, $0.betrag) })
        for d in snap.lebensmittel {
            let k = posKey(d.datum, d.ort, d.betrag)
            if verbrauche(k, &lmBestand) {
                skip += 1
                continue
            }
            context.insert(GroceryEntry(datum: d.datum, betrag: d.betrag, ort: d.ort))
            neu += 1
        }

        var anBestand = zaehleKeys(
            try context.fetch(FetchDescriptor<PurchaseEntry>()).map { posKey($0.datum, $0.bezeichnung, $0.preis) })
        for d in snap.anschaffungen {
            let k = posKey(d.datum, d.bezeichnung, d.preis)
            if verbrauche(k, &anBestand) {
                skip += 1
                continue
            }
            context.insert(
                PurchaseEntry(datum: d.datum, bezeichnung: d.bezeichnung, preis: d.preis, belegPfad: d.belegPfad))
            neu += 1
        }
        ...
        var steuerBestand = zaehleKeys(
            try context.fetch(FetchDescriptor<TaxPayment>()).map {
                "\($0.kind.rawValue)|\(Int($0.faellig.timeIntervalSince1970))|\($0.betrag)"
            })
        for d in snap.steuern {
            let k = "\(d.kind.rawValue)|\(Int(d.faellig.timeIntervalSince1970))|\(d.betrag)"
            if verbrauche(k, &steuerBestand) {
                skip += 1
                continue
            }
            context.insert(
                TaxPayment(
                    kind: d.kind, jahr: d.jahr, faellig: d.faellig, betrag: d.betrag,
                    bezahlt: d.bezahlt, bezahltAm: d.bezahltAm, bemerkung: d.bemerkung))
            neu += 1
        }

        var zuordnungKeys = Set(try context.fetch(FetchDescriptor<ZuordnungsRegel>()).map(\.schluessel))
        for d in snap.zuordnungsRegeln ?? [] {
            if zuordnungKeys.contains(d.schluessel) {
                skip += 1
                continue
            }
            ...
            context.insert(
                ZuordnungsRegel(
                    schluessel: d.schluessel, kategorie: d.kategorie,
        ...
        for d in snap.importBuchungen ?? [] {
            if importKeys.contains(d.schluessel) {
                skip += 1
                continue
            }
            ...
            context.insert(
                ImportBuchung(
                    schluessel: d.schluessel, buchungstag: d.buchungstag,
                    betrag: d.betrag, gegenpartei: d.gegenpartei, kategorie: d.kategorie,
                    betrieblich: d.betrieblich, erstellt: d.erstellt))
            neu += 1
        }
