...
enum Demodaten {
    ...
    private static func einnahmen(_ ctx: ModelContext) {
        ...
        for (nr, kunde, netto, rm, rt, zahl) in rechnungen {
            let n = dez(netto)
            let income = Income(
                kunde: kunde, rnNetto: n, ust: Steuer.ust(ausNetto: n),
        ...
        ctx.insert(
            Income(
                kunde: "Studio Ostkreuz GmbH", rnNetto: mischG, ust: Steuer.ust(ausNetto: mischG, satz: .satz19),
    ...
    private static func betrieblicheAusgaben(_ ctx: ModelContext) {
        ...
        for m in monate {
            for (bez, anb, brutto, vst, art, kind, t) in wiederkehrend {
                ctx.insert(
                    ExpenseEntry(
                        datum: tag(jahr, m, t), bezeichnung: bez, anbieter: anb,
        ...
        ctx.insert(
            ExpenseEntry(
                datum: tag(jahr, 3, 14), bezeichnung: "MacBook Air", anbieter: "Apple",
        ...
        ctx.insert(
            ExpenseEntry(
                datum: tag(jahr, 4, 9), bezeichnung: "Fachbuch Typografie", anbieter: "Buchhandlung",
    ...
    private static func zahlungen(_ ctx: ModelContext) {
        ...
        for m in 1...5 {
            ctx.insert(
                TaxPayment(
                    kind: .ksk, jahr: jahr, faellig: tag(jahr, m, 1),
        ...
        ctx.insert(
            TaxPayment(
                kind: .ustVz, jahr: jahr, faellig: tag(jahr, 5, 10),
    ...
    private static func aufgaben(_ ctx: ModelContext) {
        ctx.insert(
            MonthlyTask(
                titel: "UStVA einreichen", monat: tag(jahr, 7, 10),
                intervall: .quartalsweise, faelligTag: 10, quartalsMonate: [1, 4, 7, 10]))
        ctx.insert(
            MonthlyTask(
                titel: "Belege ablegen & prüfen", monat: tag(jahr, 6, 30),
                intervall: .monatlich, faelligTag: 30))
        ctx.insert(
            MonthlyTask(
                titel: "EÜR vorbereiten", monat: tag(jahr, 12, 31),
