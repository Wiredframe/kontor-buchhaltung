...
struct ImportTests {
    ...
    @Test func vorschlagHeuristiken() {
        #expect(
            ImportVorschlag.fuer(buchung("-100", text: "ÜBERTRAG (ÜBERWEISUNG)"), regeln: []).kategorie == .ignorieren)
        #expect(
            ImportVorschlag.fuer(buchung("3168,38", text: "GUTSCHRIFT ÜBERWEISUNG"), regeln: []).kategorie == .einnahme)
    ...
    @Test func betriebsausgabeAngelegtMitAutoVSt() throws {
        ...
        let nachricht = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .betriebsausgabe, betrieblich: true, steuerart: .inland19),
    ...
    @Test func betriebsausgabeNieAlsPrivatGebucht() throws {
        ...
        _ = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .betriebsausgabe, betrieblich: false, steuerart: .inland19),
            aktion: .neu, c.mainContext)
        ...
        let regel = try #require(
            try c.mainContext.fetch(FetchDescriptor<ZuordnungsRegel>()).first { $0.kategorie == .betriebsausgabe })
    ...
    @Test func vorschlagNormalisiertFehlerhafteRegel() throws {
        ...
        c.mainContext.insert(
            ZuordnungsRegel(schluessel: "gl:DE00ZZZFIGMA", kategorie: .betriebsausgabe, betrieblich: false))
        try c.mainContext.save()
        let regeln = try c.mainContext.fetch(FetchDescriptor<ZuordnungsRegel>())
        let z = ImportVorschlag.fuer(
            buchung("-119,00", name: "FIGMA/San Francisco/US", glaeubiger: "DE00ZZZFIGMA"), regeln: regeln)
    ...
    @Test func importErzwingtBetrieblichOhneNachtrag() throws {
        ...
        _ = try ImportAnwendung.anwenden(
            b,
            Zuordnung(
                kategorie: .betriebsausgabe, betrieblich: false,
                steuerart: .inland19),
            aktion: .neu, c.mainContext)
        let e = try #require(try c.mainContext.fetch(FetchDescriptor<ExpenseEntry>()).first)
    ...
    @Test func privatGestellteAusgabeBleibtPrivat() throws {
        ...
        c.mainContext.insert(
            ExpenseEntry(
                datum: tag(2026, 6, 1), bezeichnung: "Sneaker", anbieter: "Shop",
    ...
    @Test func haendlerErstattungMindertDieAusgabeStattSieZuErhoehen() throws {
        ...
        // Abo belastet …
        _ = try ImportAnwendung.anwenden(
            buchung("-119,00", name: "ADOBE/Dublin/IE", monat: 6, am: 1),
            z, aktion: .neu, c.mainContext)
        // … und einen Monat später storniert/erstattet.
        _ = try ImportAnwendung.anwenden(
            buchung(
                "119,00", text: "KARTENZAHLUNG GUTSCHRIFT",
    ...
    @Test func normaleBelastungBleibtPositiveAusgabe() throws {
        let c = try container()
        _ = try ImportAnwendung.anwenden(
            buchung("-119,00", name: "ADOBE/Dublin/IE"),
    ...
    @Test func ustVzImJanuarTrifftDenGeplantenVorjahresTermin() throws {
        ...
        c.mainContext.insert(
            TaxPayment(
                kind: .ustVz, jahr: 2025, faellig: tag(2026, 1, 10),
    ...
    @Test func privateFixkostenAlsBuchung() throws {
        ...
        let nachricht = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .fixkosten, betrieblich: false),
            aktion: .neu, c.mainContext)
        #expect(nachricht == "Ausgabe angelegt")
        let ausgaben = try c.mainContext.fetch(FetchDescriptor<ExpenseEntry>())
        #expect(ausgaben.count == 1)
    ...
    @Test func lerntAusEntscheidung() throws {
        ...
        _ = try ImportAnwendung.anwenden(
            b1, Zuordnung(kategorie: .lebensmittel, betrieblich: false), aktion: .neu, c.mainContext)
        // Neue ALDI-Buchung (anderer Tag/Ref) → Vorschlag kommt jetzt aus der gelernten Regel.
        let regeln = try c.mainContext.fetch(FetchDescriptor<ZuordnungsRegel>())
        let z = ImportVorschlag.fuer(
            buchung("-9,99", name: "BIOMARKT NORD/Berlin/DE", kundenref: "REF2", am: 20), regeln: regeln)
        #expect(z.kategorie == .lebensmittel)
    }
    ...
    @Test func einnahmeMatchUndBezahlt() throws {
        ...
        c.mainContext.insert(
            Income(
                kunde: "Kunde X", rnNetto: dez("100"), ust: dez("19"),
    ...
    @Test func dubletteWirdGefunden() throws {
        ...
        c.mainContext.insert(
            ExpenseEntry(
                datum: tag(2026, 6, 10), bezeichnung: "Figma", anbieter: "Figma",
        ...
        let ziel = ImportAnwendung.ziel(
            b, Zuordnung(kategorie: .betriebsausgabe, betrieblich: true, steuerart: .reverseCharge), c.mainContext)
        #expect(ziel != nil)
    }
    ...
    @Test func ausgabeMatchUeberRechnungsnummer() throws {
        ...
        c.mainContext.insert(
            ExpenseEntry(
                datum: tag(2026, 6, 1), bezeichnung: "Adobe", anbieter: "Adobe",
    ...
    @Test func ueberspringenLegtNichtsAn() throws {
        ...
        _ = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .anschaffung, betrieblich: false), aktion: .ueberspringen, c.mainContext)
    ...
    @Test func startRegelnSeedIdempotentUndVorschlag() throws {
        ...
        #expect(
            ImportVorschlag.fuer(buchung("-420,00", name: "Kuenstlersozialkasse"), regeln: regeln).kategorie == .ksk)
        ...
        let anthropic = ImportVorschlag.fuer(
            buchung("-18,00", name: "ANTHROPIC. CLAUDE SUB/San Francisco/US"), regeln: regeln)
        #expect(
            anthropic.kategorie == .betriebsausgabe && anthropic.betrieblich && anthropic.steuerart == .reverseCharge)
    }
    ...
    @Test func kskBuchungWirdAlsZahlungGebucht() throws {
        ...
        let nachricht = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .ksk, betrieblich: false), aktion: .neu, c.mainContext)
    ...
    @Test func steuererstattungAlsNegativeZahlung() throws {
        ...
        let nachricht = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .steuererstattung, betrieblich: false, steuerKind: .ustVz),
            aktion: .neu, c.mainContext)
    ...
    @Test func finanzamtVorschlagRichtungsabhaengig() throws {
        ...
        c.mainContext.insert(
            ZuordnungsRegel(
                schluessel: "finanzamt berlin", kategorie: .steuer,
        ...
        #expect(
            ImportVorschlag.fuer(buchung("305.77", name: "Finanzamt Berlin"), regeln: regeln).kategorie
                == .steuererstattung)  // Eingang → Erstattung
    ...
    @Test func steuerzahlungNeuOhneTermin() throws {
        ...
        _ = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .steuer, betrieblich: false, steuerKind: .ustVz),
            aktion: .neu, c.mainContext)
        let t = try #require(try c.mainContext.fetch(FetchDescriptor<TaxPayment>()).first)
    ...
    @Test func lerntSteuerKind() throws {
        ...
        _ = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .steuer, betrieblich: false, steuerKind: .estVz),
            aktion: .neu, c.mainContext)
        let regeln = try c.mainContext.fetch(FetchDescriptor<ZuordnungsRegel>())
        let z = ImportVorschlag.fuer(
            buchung("-1200,00", name: "Finanzamt Berlin", glaeubiger: "DEFA123", am: 20), regeln: regeln)
    ...
    @Test func reTriageIgnoriertDannKsk() throws {
        ...
        _ = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .ignorieren, betrieblich: false), aktion: .neu, c.mainContext)
    ...
    @Test func erstattungAlsNegativeAnschaffung() throws {
        ...
        let nachricht = try ImportAnwendung.anwenden(
            b, Zuordnung(kategorie: .erstattung, betrieblich: false), aktion: .neu, c.mainContext)
