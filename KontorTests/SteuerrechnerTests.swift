...
struct UStTests {
    ...
    @Test func ustVzZuordnungNachEinstellungen() {
        ...
        #expect(
            Steuer.ustVzZuordnung(zahlMonat: 2, zahlJahr: 2026, rhythmus: .vierteljaehrlich, dauerfrist: false).jahr
                == 2026)
        // mit Dauerfristverlängerung: Februar → Q4 Vorjahr.
        #expect(
            Steuer.ustVzZuordnung(zahlMonat: 2, zahlJahr: 2026, rhythmus: .vierteljaehrlich, dauerfrist: true).jahr
                == 2025)
        ...
        #expect(
            Steuer.ustVzZuordnung(zahlMonat: 4, zahlJahr: 2026, rhythmus: .vierteljaehrlich, dauerfrist: false).jahr
                == 2026)
    }
}
...
struct UStVATests {
    ...
    static func q1Ausgaben2026() -> [AusgabePosten] {
        func a(_ m: Int, _ tg: Int, _ brutto: String, _ vst: String, _ art: Steuerart) -> AusgabePosten {
            AusgabePosten(
                brutto: dez(brutto), vst: dez(vst), steuerart: art, betrieblich: true, datum: tag(2026, m, tg))
        }
        return [
            // Januar
    ...
    @Test func anschaffungNettoHardware() {
        let hardware = AusgabePosten(
            brutto: dez("1190.00"), vst: dez("190.00"),
            steuerart: .inland19,
    ...
    @Test func ustSollNurNachRechnungsdatum() {
        let rechnungen = [
            EinnahmePosten(
                rnNetto: dez("1000"), ust: dez("190"), rechnungsdatum: tag(2026, 3, 31),
                zahlungsdatum: nil, status: .offen, ausfalldatum: nil),
            EinnahmePosten(
                rnNetto: dez("1000"), ust: dez("190"), rechnungsdatum: tag(2026, 4, 1),
                zahlungsdatum: nil, status: .offen, ausfalldatum: nil),
        ]
    ...
    @Test func formularKennzahlen() {
        let rechnungen = [
            EinnahmePosten(
                rnNetto: dez("1000"), ust: dez("190"), rechnungsdatum: tag(2026, 2, 10),
                zahlungsdatum: nil, status: .offen, ausfalldatum: nil),
            // steuerfrei (USt 0) → bleibt aus KZ 81 heraus
            EinnahmePosten(
                rnNetto: dez("500"), ust: dez("0"), rechnungsdatum: tag(2026, 2, 12),
                zahlungsdatum: nil, status: .offen, ausfalldatum: nil),
        ]
...
struct JahresAggregatTests {
    static let einnahmen = [
        EinnahmePosten(
            rnNetto: dez("4000.00"), ust: dez("760.00"), rechnungsdatum: tag(2026, 3, 15),
            zahlungsdatum: nil, status: .offen, ausfalldatum: nil)
    ]
...
struct AusfallTests {
    private static let rechnung = EinnahmePosten(
        rnNetto: dez("5000"), ust: dez("950"),
    ...
    @Test func ausfallEinerErmaessigtenRechnungMindertKZ86() {
        let r = EinnahmePosten(
            rnNetto: dez("1000"), ust: dez("70"), satz: .satz7,
            rechnungsdatum: tag(2025, 11, 10), zahlungsdatum: nil,
            status: .ausgefallen, ausfalldatum: tag(2026, 2, 15))
    ...
    @Test func ausfallUndUmsatzImSelbenQuartalSaldieren() {
        let laufend = EinnahmePosten(
            rnNetto: dez("8000"), ust: dez("1520"),
...
struct ESTGrundfreibetragTests {
    ...
    @Test func voraussichtlichMitFreibetrag() {
        #expect(
            Steuer.estVoraussichtlich(
                gewinn: dez("18000.00"), ksk: dez("2400.00"),
    ...
    @Test func voraussichtlichNieNegativ() {
        #expect(
            Steuer.estVoraussichtlich(
                gewinn: dez("10000.00"), ksk: dez("2000.00"),
    ...
    @Test func freibetragSenktGegenueberPauschal() {
        ...
        let mitFreibetrag = Steuer.estVoraussichtlich(
            gewinn: gewinn, ksk: ksk,
...
struct EUERTests {
    @Test func gewinnNachZuflussUndNurBetrieblich() {
        let einnahmen = [
            ...
            EinnahmePosten(
                rnNetto: dez("2400"), ust: dez("456"), rechnungsdatum: tag(2026, 5, 5),
                zahlungsdatum: tag(2026, 5, 28), status: .bezahlt, ausfalldatum: nil),
            // offen (kein Zahlungseingang) → zählt nicht
            EinnahmePosten(
                rnNetto: dez("1000"), ust: dez("190"), rechnungsdatum: tag(2026, 6, 2),
                zahlungsdatum: nil, status: .offen, ausfalldatum: nil),
            // 2025 gestellt, aber 2026 bezahlt → zählt 2026 (Zufluss)
            EinnahmePosten(
                rnNetto: dez("500"), ust: dez("95"), rechnungsdatum: tag(2025, 12, 10),
        ...
        let ausgaben = [
            ...
            AusgabePosten(
                brutto: dez("50"), vst: dez("0"), steuerart: .steuerfrei,
...
struct RuecklageTests {
    ...
    @Test func ruecklageUndVerfuegbarMaerzBisJuni() {
        ...
        let monate: [M] = [
            M(
                ust: dez("800"), vst: dez("200"), ksk: dez("420"), est: dez("600"), fix: dez("900"),
                brutto: dez("5000"), ruecklage: dez("2520"), verfuegbar: dez("2480")),
            M(
                ust: dez("600"), vst: dez("100"), ksk: dez("420"), est: dez("400"), fix: dez("900"),
                brutto: dez("4000"), ruecklage: dez("2220"), verfuegbar: dez("1780")),
            M(
                ust: dez("475"), vst: dez("50"), ksk: dez("500"), est: dez("300"), fix: dez("900"), brutto: dez("3000"),
                ruecklage: dez("2125"), verfuegbar: dez("875")),
            M(
                ust: dez("1000"), vst: dez("50"), ksk: dez("500"), est: dez("800"), fix: dez("900"),
                brutto: dez("6500"), ruecklage: dez("3150"), verfuegbar: dez("3350")),
        ]
        for m in monate {
            let steuer = Steuer.steuerRuecklage(
                ust: m.ust, vorsteuer: m.vst, ustKorrektur: 0, ksk: m.ksk, estAnteil: m.est)
    ...
    @Test func verfuegbarIstGewinnNachBetrieblichenUndPrivatenAusgaben() {
        let einnahmen = [
            EinnahmePosten(
                rnNetto: dez("4000"), ust: dez("760"),
                rechnungsdatum: tag(2026, 6, 5), zahlungsdatum: nil,
                status: .offen, ausfalldatum: nil)
        ]
        let ausgaben = [
            AusgabePosten(
                brutto: dez("1190"), vst: dez("190"), steuerart: .inland19,
                betrieblich: true, datum: tag(2026, 6, 10))
        ]  // netto 1000
    ...
    @Test func estAusfallNimmtRechnungsmonatsSatz() {
        let einnahmen = [
            EinnahmePosten(
                rnNetto: dez("1000"), ust: 0, rechnungsdatum: tag(2026, 3, 10),
        ...
        let a = Steuer.monatsauswertung(
            monat: 9, jahr: 2026, einnahmen: einnahmen, ausgaben: [],
            kskFuer: { _, _ in 0 },
...
struct EStAusfallTests {
    ...
    @Test func ausfallInVerlustmonatLoestNichtsAuf() {
        let einnahmen = [
            EinnahmePosten(
                rnNetto: dez("1000"), ust: 0, rechnungsdatum: tag(2026, 5, 10),
                zahlungsdatum: nil, status: .ausgefallen, ausfalldatum: tag(2026, 8, 15))
        ]
        let ausgaben = [
            AusgabePosten(
                brutto: dez("1071"), vst: dez("171"), steuerart: .inland19,
                betrieblich: true, datum: tag(2026, 5, 20))
        ]  // netto 900
        // Mai: max(0, (1000 − 900) − 300) × 15 % = 0 → nichts gebildet, nichts aufzulösen.
        let a = Steuer.monatsauswertung(
            monat: 8, jahr: 2026, einnahmen: einnahmen, ausgaben: ausgaben,
    ...
    @Test func teilausfallLoestAnteiligAuf() {
        let einnahmen = [
            EinnahmePosten(
                rnNetto: dez("600"), ust: 0, rechnungsdatum: tag(2026, 5, 10),
                zahlungsdatum: nil, status: .ausgefallen, ausfalldatum: tag(2026, 6, 15)),
            EinnahmePosten(
                rnNetto: dez("400"), ust: 0, rechnungsdatum: tag(2026, 5, 12),
    ...
    @Test func mehrereAusfaelleEinesMonatsUebersteigenDieBildungNicht() {
        ...
        let satz: (Int, Int) -> Decimal = { _, _ in dez("0.15") }
        let mai = Steuer.monatsauswertung(
            monat: 5, jahr: 2026, einnahmen: einnahmen, ausgaben: [],
            kskFuer: ksk, fixkostenPrivat: 0, pauschalSatz: satz)
        let jun = Steuer.monatsauswertung(
            monat: 6, jahr: 2026, einnahmen: einnahmen, ausgaben: [],
            kskFuer: ksk, fixkostenPrivat: 0, pauschalSatz: satz)
        let aug = Steuer.monatsauswertung(
            monat: 8, jahr: 2026, einnahmen: einnahmen, ausgaben: [],
    ...
    @Test func estRuecklageJahrBleibtBeiVerlustmonatNichtNegativ() {
        ...
        let est = Steuer.estRuecklageJahr(
            jahr: 2026, einnahmen: einnahmen, ausgaben: ausgaben,
    ...
    @Test func estAusfallNimmtAusgabenDesRechnungsmonats() {
        ...
        let ausgaben = [
            AusgabePosten(
                brutto: dez("714"), vst: dez("114"), steuerart: .inland19,
    ...
    @Test func jahresuebergreifenderAusfallNimmtWerteDesRechnungsjahres() {
        let einnahmen = [
            EinnahmePosten(
                rnNetto: dez("5000"), ust: 0, rechnungsdatum: tag(2025, 11, 10),
                zahlungsdatum: nil, status: .ausgefallen, ausfalldatum: tag(2026, 2, 12))
        ]
        let ausgaben = [
            AusgabePosten(
                brutto: dez("1190"), vst: dez("190"), steuerart: .inland19,
                betrieblich: true, datum: tag(2025, 11, 20))
        ]  // netto 1000
        // Nov 2025: (5000 − 1000 − 500) × 19 % = 665
        let a = Steuer.monatsauswertung(
            monat: 2, jahr: 2026, einnahmen: einnahmen, ausgaben: ausgaben,
    ...
    @Test func anteiligeAufloesungRundetKaufmaennisch() {
        let einnahmen = [
            EinnahmePosten(
                rnNetto: dez("333.33"), ust: 0, rechnungsdatum: tag(2026, 5, 10),
                zahlungsdatum: nil, status: .ausgefallen, ausfalldatum: tag(2026, 6, 15)),
            EinnahmePosten(
                rnNetto: dez("666.67"), ust: 0, rechnungsdatum: tag(2026, 5, 12),
                zahlungsdatum: nil, status: .ausgefallen, ausfalldatum: tag(2026, 8, 20)),
        ]
        let satz: (Int, Int) -> Decimal = { _, _ in dez("0.15") }
        let mai = Steuer.monatsauswertung(
...
struct MehrsatzTests {
    private func e(
        _ netto: String, _ ust: String, _ satz: UStSatz, _ status: InvoiceStatus = .offen,
        ausfall: Date? = nil, rechnung: Date = tag(2026, 2, 10)
    ) -> EinnahmePosten {
        EinnahmePosten(
            rnNetto: dez(netto), ust: dez(ust), satz: satz, rechnungsdatum: rechnung,
    ...
    @Test func mischrechnungTraegtInBeideSaetze() {
        let inc = Income(
            kunde: "X", rnNetto: dez("2000"), ust: dez("380"), rechnungsdatum: tag(2026, 2, 10),
    ...
    @Test func ausgabe7ProzentInKz66UndEUER() {
        let a = [
            AusgabePosten(
                brutto: dez("107"), vst: dez("7"), steuerart: .inland7,
